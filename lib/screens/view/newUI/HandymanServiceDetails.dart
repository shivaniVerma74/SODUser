...
import 'HandyServiceCart.dart';
import 'Services_Cart.dart';

class HandyServiceDetails extends StatefulWidget {
  String? h_id;
   HandyServiceDetails({Key? key, this.h_id}) : super(key: key);

  @override
  State<HandyServiceDetails> createState() => _HandyServiceDetailsState();
}

class _HandyServiceDetailsState extends State<HandyServiceDetails> {
  @override
  void initState() {
  ...
  getHandymanServices() async {
    ...
    print("Hnadyman servicesss apiii in detailsss screen");
    var headers = {
    ...
    print("get handyman services by id ${request.fields}");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var finalResponse = await response.stream.bytesToString();
      final jsonResponse = HandyServicesModel.fromJson(json.decode(finalResponse));
      print("handyman servicessss in details pageee$jsonResponse");
      if(jsonResponse.status == 1){
        ...
        print("handymann product id nowww ${product_id}");
        print("handymannn vendor id nowwww ${vendor_id}");
        setState(() {
  ...
  addToCartByServices() async{
    ...
    if (response.statusCode == 200) {
      ...
      if(jsonResponse.status == true){
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) => HandyManCart()));
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        backgroundColor: appColorWhite,
        title:Text("HANDYMAN SERVICE DETAILS", style: TextStyle(color: appColorBlack, fontSize: 10, fontFamily: 'versailles', decoration: TextDecoration.underline, fontWeight: FontWeight.bold )),
      ),
      body: Stack(
        children: [
          Container(
                            ...
                            child: Card(
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    SizedBox(width: 2),
                                    Center(
                                      child: handyServicesModel!.data![index].profileImage == null ||handyServicesModel!.data![index].profileImage == ""? Center(
                                        child: Text("No Image Data", style: TextStyle(fontSize: 14, color: backgroundblack),)
                                      ) : Image.network("${handyServicesModel!.data![index].profileImage}", height: 100, width: 100, fit: BoxFit.fill,),
                                    ),
                                    Padding(
                                      ...
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text("${handyServicesModel!.data![index].artistName}",style: TextStyle(color: backgroundblack,fontWeight: FontWeight.bold,fontSize: 13),),
                                          Text("${handyServicesModel!.data![index].serDesc}", style: TextStyle(color: appColorBlack,fontSize: 13)),
                                          Row(
                                            children: [
                                              Text("₹${handyServicesModel!.data![index].mrpPrice}", style: TextStyle(color: appColorOrange, fontSize: 13, decoration: TextDecoration.lineThrough),),
                                              SizedBox(width: 4,),
                                              Text("₹${handyServicesModel!.data![index].specialPrice}", style: TextStyle(color: appColorBlack,fontSize: 13, fontWeight: FontWeight.bold)
                                              ),
                                            ],
                                          ),
                                          Row(
                                            children: [
                                              ...
                                              Text("${handyServicesModel!.data![index].uname}", style: TextStyle(fontSize: 13, color: appColorBlack, fontWeight: FontWeight.bold),),
                                            ],
                                          ),
                                          SizedBox(height: 10),
                                          Row(
                                            children: [
                                              InkWell(
                                                onTap: () {
                                                  // Navigator.push(context, MaterialPageRoute(builder: (context) => HandyserviceCart()));
                                                  addToCartByServices();
                                                },
                                                child: Container(
                                                  height: 30,
                                              ...
                                              SizedBox(width: 80),
                                              handyServicesModel!.data![index].gender == 'Female' ? Image.asset("assets/images/girl.png", scale: 2) :
                                              handyServicesModel!.data![index].gender == 'Male' ? Image.asset("asssets/images/boy.png", scale: 2,) :
                                              handyServicesModel!.data![index].gender == 'Both' ? Image.asset("assets/images/girl.png", scale: 2,): Image.asset("assets/images/boy.png", scale: 2,)
                                            ],
                                          ),
                                          // Row(
